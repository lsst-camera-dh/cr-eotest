...
#            pd2 = flat2.md.get('MONDIODE')
            ...
            if (exptime1==0.0 or exptime2==0.0) :
                continue

#            pdfile1 = file1.split('.')[0]+'_pdvals.txt'
            pdfile1 = file1[0:len(file1)-5]+'_pdvals.txt'
            fp = open(pdfile1,"r");
            pd1 = 0.0
            period = 0.0
            nsamps = 0.0
            for line in fp:
                pd1 += float(line.split()[1]) / 1.0e-9
                period = float(line.split()[0])
                nsamps += 1.0
            if exptime1 > 0.0 :
                pd1 /= nsamps
                pd1 = pd1 * period/exptime1
            else :
                pd1 = 0.0
            print("mondiode1 = ",pd1)
            fp.close()

#            pdfile2 = file2.split('.')[0]+'_pdvals.txt'
            pdfile2 = file2[0:len(file1)-5]+'_pdvals.txt'
            fp = open(pdfile2,"r");
            pd2 = 0.0
            period = 0.0
            nsamps = 0.0
            for line in fp:
                pd2 += float(line.split()[1]) / 1.0e-9
                period = float(line.split()[0])
                nsamps += 1.0
            if exptime2 > 0.0 :
                pd2 /= nsamps
                pd2 = pd2 * period/exptime2
            else :
                pd2 = 0.0
            print("mondiode2 = ",pd2)
            fp.close()
